      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <h2
            ...
            style={{ fontSize: "clamp(32px, 5vw, 54px)", letterSpacing: "-0.04em", color: "var(--cream)" }}
          >
            Nos services
          </h2>
        </motion.div>
        ...
        >
          {services.map((svc, i) => (
            <motion.div
              ...
              style={{
                background: "var(--ink)",
                borderRight: i === 0 ? "1px solid var(--ink-4)" : undefined,
              }}
              custom={i}
              variants={cardVariants}
              initial="hidden"
              animate={inView ? "visible" : "hidden"}
              whileHover={{ backgroundColor: "#191A1F" }}
              transition={{ duration: 0.3 }}
            >
              <Link href={svc.href} className="absolute inset-0" aria-label={svc.ariaLabel} />
              <div className="flex items-start justify-between mb-6">
                <p className="font-mono text-[10px] uppercase tracking-wider" style={{ color: "var(--cream-faint)" }}>
                  {svc.num}
                </p>
                <motion.span
